class Dashing.Clock extends Dashing.Widget
  ...
  startTime: =>
    ...
    if offset2
      ...
      @set('time2', @formatTime(time2.getHours()) + ":" + @formatTime(time2.getMinutes()) + ":" + @formatTime(time2.getSeconds()) + " - Denver")

    offset3 = $(@node).data('third-time-offset')
    if offset3
      time3 = new Date()
      time3.setTime(today.getTime()+offset3*60*60*1000)
      @set('time3', @formatTime(time3.getHours()) + ":" + @formatTime(time3.getMinutes()) + ":" + @formatTime(time3.getSeconds()) + " - LA")
